...
- 응답에 쓰는 `ReadingRecord` 타입과 `ReadingStatus` 값 목록은 모두 `src/generated/prisma/client`에서 import합니다.
- `src/reading-records/dto/` 아래 DTO들은 여전히 API 입력 형태를 따로 표현합니다 — 아직 응답 전용 타입은 분리하지 않았습니다.
